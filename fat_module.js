...
(function(){
  ...
  style.textContent = `
    ...
    .fat-fa .tail{ position:absolute; left:100px; top:60px; width:200px; height:32px; border-radius:10px; background:#fef08a; border:1px solid #d97706; display:flex; align-items:center; justify-content:center; font-weight:900; color:#7c2d12; font-size:16px; white-space:nowrap; }
    ...
    .fat-fa .cooh::after{ content:''; position:absolute; left:100%; top:50%; width:40px; height:2px; background:#475569; transform:translateY(-50%); }
    /* 肥皂徽标 */
      ...
      for(const fa of items.filter(i=>i.type==='fa')){
        ...
        for(let gi=1; gi<=3; gi++){
          if(!gly.used[gi-1]){
            ...
            if(!p1||!p2) continue;
            // 使用元素边界框相交来判定是否可以脱水缩合，只要两端口重叠即可
