...
(function(){
  ...
  style.textContent = `
    ...
    .fat-fa .tail{ position:absolute; left:100px; top:60px; width:300px; height:40px; border-radius:10px; background:#fef08a; border:1px solid #d97706; display:flex; align-items:center; justify-content:center; font-weight:900; color:#7c2d12; font-size:18px; white-space:nowrap; }
        ...
        for(let gi=1; gi<=3; gi++){
          if(!gly.used[gi-1]){
            ...
            if(ohEl && coohEl){
              ...
              // 如果中心距离小于 80px，则认为两官能团靠近，可以形成酯键
              if(Math.hypot(cx1 - cx2, cy1 - cy2) < 80){
                formEster(gly, gi, fa);
                return;
              }
            }
